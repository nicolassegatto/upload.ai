...
export function VideoInputForm(props: VideoInputFormProps) {
  ...
  async function convertVideoToAudio(video: File) {
    ...
    await ffmpeg.exec([
      '-i', 'input.mp4', '-map', '0:a', '-b:a', '16k', '-acodec', 'libmp3lame', 'output.mp3'
    ])
  ...
  return (
    <form className="space-y-6" onSubmit={handleUploadVideo}>
      ...
      >
        {previewURL ? (
          ...
          <>
            ...
            Carregar audio/video
          </>
        )}
      </label>
      <input type="file" id="video" accept="video/mp4, audio/*" className="sr-only" onChange={handleFileSelected} />
      ...
      <Button 
        disabled={status !== "waiting"} 
        type="submit" 
        className="w-full data-[success=true]:bg-blue-400"
